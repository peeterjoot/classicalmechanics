...
%\section{Motivation.}
%
\makeproblem{}{problem:jackson12Dash9:1}{
In \citep{jackson1975cew}, the non-covariant Lagrangian for the Lorentz equation is given as
%
\begin{equation}\label{eqn:jacksonLorLag:foo1}
L = - m c^2 \sqrt{1 -\Bu^2/c^2} + \frac{e}{c} \Bu \cdot \BA - e \phi.
\end{equation}
%
Evaluate this to show that this 
produces the Lorentz force law.
%  Felt like trying this anew since I recall having trouble the first time I tried it (the covariant derivation was easier).
%
} % problem
\makeanswer{problem:jackson12Dash9:1}{
%
...
\spacegrad \LL = \frac{d}{dt} \spacegrad_\Bu \LL = \left( \frac{\partial}{\partial t} + \Bu \cdot \spacegrad \right) \Be_a \frac{\partial \LL}{\partial \xdot^a}.
\end{equation}
%
where \(\{\Be_a\}\) is the spatial basis.  The first order of business is calculating the gradient and conjugate momenta.  For the latter we have
%
\begin{equation}\label{eqn:jackson12Dash9:23}
\begin{aligned}
\Be_a \frac{\partial \LL}{\partial \xdot^a}
&=
\Be_a \left(- m c^2 \gamma \inv{2} (-2) \xdot^a/c^2 + \frac{e}{c} A^a \right) \\
&=
...
\Be_a \frac{\partial \LL}{\partial x^a} = e\left( \inv{c}\xdot^b \spacegrad A^b - \spacegrad \phi \right).
\end{equation}
%
