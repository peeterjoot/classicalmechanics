...
\Lq = \inv{2} m r^2 \left( \dot{\theta}^2 + \dot{\phi}^2 \sin^2 \theta \right) - m g r ( 1 + \cos\theta ),
\end{equation}
%
...
\begin{aligned}
p_\theta &= \PD{\dot{\theta}}{\Lq} = m r^2 \dot{\theta} \\
p_\phi &= \PD{\dot{\phi}}{\Lq} = m r^2 \sin^2\theta \dot{\phi}.
\end{aligned}
\end{equation}
%
That is enough to now formulate the Hamiltonian \(H = \dot{\theta} p_\theta + \dot{\phi} p_\phi - \Lq\), which is
%
...
\Lq = \inv{2} m (\dot{x}^2 + \dot{y}^2 ) - V(x).
\end{equation}
%
...
\begin{aligned}
\Lq &= \inv{2} m \left(\dot{r}^2 + r^2 \dot{\theta}^2 \right) - V(r) \\
p_r &= m \dot{r} \\
...
R = R(q_1, \cdots q_n, p_1, \cdots p_s, \dot{q}_{s+1}, \cdots \dot{q}_n, t) = \sum_{i=1}^{s} \dot{q}_i p_i - \Lq.
\end{equation}
%
...
- \sum_{i=1}^s \PD{q_i}{\Lq} dq_i
- \sum_{i=1}^n \PD{\dot{q}_i}{\Lq} d\dot{q}_i
- \PD{t}{\Lq} dt \\
&=
\PD{p_i}{R} dp_i
+ \PD{q_i}{R} dq_i
