...
%%%{scale=0.6}
%%%
%%%The tangent space basis at a specific point of a two parameter surface, \( x(u^0, u^1) \), is illustrated in
...
%%%\Bx^\mu = \grad u^\mu.
%%%\end{equation}
%%%The vectors \( \Bx^\mu \) are called the reciprocal frame vectors, and the ordered set \( \setlr{ \Bx^0, \Bx^1, \Bx^2, \Bx^3 } \) is called the reciprocal basis.
%%%
...
%%%For the parameterization of \cref{eqn:reciprocalblog:2360}, find the reciprocal frame vectors by matrix inversion.
%%%} % problem
%%%\makeanswer{problem:reciprocal:1400}{
%%%We expanded \( \Bx_1 \) explicitly in \cref{eqn:reciprocalblog:1200}.  Doing the same for \( \Bx_2 \), we have
...
%%%\end{aligned}
%%%\end{equation}
%%%This matches \cref{eqn:reciprocalblog:1380}, as expected, but required only algebraic work to compute.
%%%} % answer
...
%%%\end{aligned}
%%%\end{equation}
%%%We can compute the reciprocals of these vectors using the gradient method.  It's possible to state matrix equations representing the reciprocal relationship of \cref{eqn:reciprocalblog:2200}, which, in this case, is \( X^\T G Y = 1 \), where the RHS is a \( 2 \times 2 \) identity matrix, and \( X, Y\) are \( 4\times 2\) matrices of coordinates, with
...
%%%``factored'' out vector is reincorporated into the pseudoscalar using the distribution identity \cref{eqn:reciprocalblog:2280}.
