...
%\section{Curvilinear coordinates and gradient in spacetime, and reciprocal frames.}
%\generatetitle{A couple more reciprocal frame examples.}
...
and the tangent space basis at a specific point of a two parameter surface, \( x(u^0, u^1) \), is also illustrated in 
\cref{fig:twoParameterDifferentialCov:twoParameterDifferentialCovFig1}
%\cref{fig:tangentSpaceSurface:tangentSpaceSurfaceFig1}
.  The differential directions that span the tangent space are
...
%\threeFiguresOneLineNonFloat
%{../figures/GAelectrodynamics/bw/oneParameterDifferentialArrowsFig1}
%{../figures/GAelectrodynamics/bw/tangentSpaceSurfaceFig1}
%{../figures/GAelectrodynamics/twoParameterDifferentialCovFig1}
%{Tangent spaces and parameterized curves.}
%{fig:tangentSpaceSurface:tangentSpaceSurfaceFig1}
%{scale=0.6}
...
Later we will see that parameterization constraints must be imposed, as not all surfaces generated by a set of parameterizations are useful for integration theory.  In particular, degenerate parameterizations for which the wedge products of the tangent space basis vectors are zero, or those wedge products cannot be inverted, are not physically meaningful.  We require the functional form of parameterizations to be differentiable, to provide a one to one invertible mapping from the parameter space to the vector space.
% and linearly independent vectors on both ends of the transformation
Properly behaved surfaces of this sort are called manifolds.
...
\end{dmath}
\end{proof}
The following example parameterization scales the proper time parameter, and uses polar coordinates in the \(x,y\) plane.
...
\end{dmath}
so \( x = x^\mu \gamma_\mu = c t \gamma_0 + \gamma_1 \rho e^{-i \theta} + z \gamma_3 \) is equivalent to the following set of coordinate equations
...
Despite being a fairly simple parameterization, it was still fairly difficult to solve for the gradients when the parameterization introduced coupling between the coordinates.
Because all the tangent space vectors are mutually orthogonal, we didn't need to go through all this trouble, since we could have just computed the inverses of all the tangent space vectors
\begin{equation}\label{eqn:reciprocal:1940}
\begin{aligned}
\Bx^0 &= \inv{\Bx_0} = \inv{ c \gamma_0 } = \inv{c} \gamma^0 \\
\Bx^1 &= \inv{\Bx_1} = \inv{ \gamma_1 e^{-i\alpha} } = \inv{ e^{-i\alpha} } \inv{ \gamma_1} = e^{i \alpha} \gamma^1 = \gamma^1 e^{-i\alpha} \\
\Bx^2 &= \inv{\Bx_2} = \inv{ \rho \gamma_2 e^{-i\alpha} } = \inv{\rho} \inv{ e^{-i\alpha} } \inv{ \gamma_2} = \inv{\rho} e^{i \alpha} \gamma^2 = \inv{\rho} \gamma^2 e^{-i\alpha} \\
\Bx^3 &= \inv{\Bx_3} = \inv{ \gamma_3 } = \gamma^3.
\end{aligned}
\end{equation}
%In this particular case, we could have solved for the parameters in terms of the coordinates (but it was easier not to), but that will not generally be true.
We will not generally be this lucky, and want a less labor intensive strategy to find the reciprocal frame that works for the general case.

There is one additional special cawe.  When we have a full parameterization of spacetime, then we can do this with nothing more than a matrix inversion.
...
\makeproblem{Unidirectional arbitrary functional dependence.}{problem:reciprocal:13}{
Let
\begin{equation*}
x = a f(u),
\end{equation*}
...
\begin{dmath}\label{eqn:reciprocal:2420}
\inv{a} \cdot x = f(u),
\end{dmath}
...
\makeproblem{Linear two variable parameterization.}{problem:reciprocal:1960}{
...
\makeanswer{problem:reciprocal:1960}{
...
\makeproblem{Quadratic two variable parameterization.}{problem:reciprocal:1980}{
...
\makeanswer{problem:reciprocal:1980}{
...
\end{aligned}
\end{equation}
A bit of rearrangement, shows that we recover \cref{eqn:reciprocal:2080} easily.  This is a second demonstration that the gradient and the algebraic formulations for the reciprocals match, at least for these special cases of linear non-coupled parameterizations.
} % answer
\makeproblem{Reciprocal frame for generalized cylindrical parameterization.}{problem:reciprocal:17}{
...
Note that this is cylindrical parameterization for the rotation case, and traces out hyperbolic regions for the boost case.  The boost case is illustrated in \cref{fig:boostsCylindrical:boostsCylindricalFig1} where hyperbolas in the light cone are found for boosts of \( \gamma_0\) with various values of \(\rho\), and the spacelike hyperbolas are boosts of \( \gamma_1 \), again for various values of \( \rho \).
} % problem
\imageFigure{../figures/classicalmechanics/boostsCylindricalFig1}{``Cylindrical'' boost parameterization.}{fig:boostsCylindrical:boostsCylindricalFig1}{0.3}
\makeanswer{problem:reciprocal:17}{
The tangent space vectors are
\begin{dmath}\label{eqn:reciprocal:2480}
\Bx_\rho = \frac{x}{\rho},
\end{dmath}
and
%with \( x_0 = x(\rho_0, \theta_0) \)
\begin{dmath}\label{eqn:reciprocal:2500}
\Bx_\theta
= -\frac{i}{2} x + x \frac{i}{2}
= x \cdot i.
\end{dmath}
Recall that \( x \cdot i \) lies perpendicular to \( x \) (in the plane \( i \)), as illustrated in \cref{fig:projectionsAndRejections:projectionsAndRejectionsFig1}.  This means that \( \Bx_\rho \) and \( \Bx_\theta \) are orthogonal, so we can find the reciprocal vectors by just inverting them
...
%%%\makeproblem{Linear two variable coupled quadratic parameterization.}{problem:reciprocal:1990}{
%%%Let \( x = u^2 a + 2 u v b + v^2 c \), where \( a,b,c\) are all vectors.  Compute the tangent space basis, and algebraically compute the reciprocal vectors.
%%%} % problem
%%%\makeanswer{problem:reciprocal:1990}{
%%%The tangent space vectors are computed easily
%%%\begin{equation}\label{eqn:reciprocal:2140}
%%%\begin{aligned}
%%%\Bx_u &= \PD{u}{x} = 2 \lr{ u a + v b } \\
%%%\Bx_v &= \PD{v}{x} = 2 \lr{ u b + v c }.
%%%\end{aligned}
%%%\end{equation}
%%%} % answer
\subsection{Parameterization of a general linear transformation.}
...
\begin{dmath}\label{eqn:reciprocal:2180}
\Bx_\mu
= \PD{u^\mu}{x}
= \PD{u^\mu}{}
{a^\alpha}_\beta \gamma_\alpha u^\beta
...
\begin{dmath}\label{eqn:reciprocal:2200}
\Bx_\mu \cdot \gamma^\nu
=
...
\end{dmath}
so
\begin{dmath}\label{eqn:reciprocal:2220}
x = \Bx_\mu u^\mu.
\end{dmath}
We are able to reinterpret \cref{eqn:reciprocal:2160} as a contraction of the tangent space vectors with the parameters, scaling and summing these direction vectors to characterize all the points in the tangent plane.
\maketheorem{Projecting onto the tangent space.}{thm:reciprocal:2240}{
Let \( T \) represent the tangent space.  The projection of a vector onto the tangent space has the form
\begin{equation*}
\Proj_T y = \lr{ y \cdot \Bx^\mu } \Bx_\mu = \lr{ y \cdot \Bx_\mu } \Bx^\mu.
\end{equation*}
} % theorem
\begin{proof}
Let's designate \( a \) as the portion of the vector \( y \) that lies outside of the tangent space
\begin{dmath}\label{eqn:reciprocal:2260}
y = y^\mu \Bx_\mu + a.
\end{dmath}
...
\begin{dmath}\label{eqn:reciprocal:2280}
y \cdot \Bx^\nu
= \lr{ y^\mu \Bx_\mu + a } \cdot \Bx^\nu
= y^\nu,
\end{dmath}
so
\begin{dmath}\label{eqn:reciprocal:2300}
...
\begin{dmath}\label{eqn:reciprocal:2320}
y = y_\mu \Bx^\mu + a.
\end{dmath}
Dotting with \( \Bx_\mu \), we have
\begin{dmath}\label{eqn:reciprocal:2340}
y \cdot \Bx^\mu
= \lr{ y_\alpha \Bx^\alpha + a } \cdot \Bx_\mu
= y_\mu,
\end{dmath}
so
\begin{dmath}\label{eqn:reciprocal:2360}
y = \lr{ y \cdot \Bx^\mu } \Bx_\mu + a.
\end{dmath}
...
\begin{dmath}\label{eqn:reciprocal:2380}
