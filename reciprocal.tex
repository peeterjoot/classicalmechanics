...
\end{subequations}
\end{proof}
Geometrically \( \Bx_1 \wedge \Bx_2 = 0 \) means that the pair of tangent space vectors are colinear.  Clearly, such a degenerate parameterization undesirable, and we have to assume that there is no such zero area element in the region of interest.

Since we are working in a mixed signature space, where we have the possibility of vectors that square to zero (i.e. lightlike), we have an additional sort of possible degeneracy.
We saw that \cref{thm:reciprocal:1500} depends on a non-zero square for the \( \Bx_1 \wedge \Bx_2 \) bivector.  If a bivector can be factored into a pair of orthogonal vectors, and either one of those is lightlike, then the bivector will square to zero, and this method of computing the reciprocal frame is invalid.  Such a degenerate parameterization will likely lead to many other troubles, so we should watch out that we don't construct such a beast.
