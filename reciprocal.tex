...
\makeproblem{Unidirectional arbitary functional dependence.}{problem:reciprocal:13}{
Let
\begin{equation*}
x = a f(u),
\end{equation*}
...
\imageFigure{../figures/classicalmechanics/boostsCylindricalFig1}{``Cylindical'' boost parameterization.}{fig:boostsCylindrical:boostsCylindricalFig1}{0.3}
} % problem
...
= x \cdot i.
\end{dmath}
Recall that \( x \cdot i \) lies perpendicular to \( x \) (in the plane \( i \)), as illustrated in \cref{fig:projectionsAndRejections:projectionsAndRejectionsFig1}.  This means that \( \Bx_\rho \) and \( \Bx_\theta \) are orthonagonal, so we can find the reciprocal vectors by just inverting them
...
%%%\end{equation}
%%%} % answer
\section{Parameterization of a general linear transformation.}
...
y = y^\mu \Bx_\mu + a.
\end{dmath}
If we knew the coordinates \( y^\mu \), we would have a reciepe for the projection.
...
\end{dmath}
Similarily, the tangent space projection can be expressed as a linear combination of reciprocal basis elements
