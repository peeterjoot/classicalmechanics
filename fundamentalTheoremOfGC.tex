...
\end{equation}
where \(F,G\) are multivectors functions (i.e. sums of products of vectors.)  We've seen (\citep{aMacdonaldVAGC}, \citep{pjootGAEE}) that all the theorems above are special cases of the fundamental theorem.  Do we need any special care to state the FTGC correctly for our relativistic case?  It turns out that the answer is no!  Tangent and reciprocal frame vectors do all the heavy lifting, and we can use the fundamental theorem as is, even in our mixed signature space.  The only real change that we need to make is use spacetime gradient and vector derivative operators instead of their spatial equivalents.  We will see how this works below.  Note that instead of starting with \cref{eqn:fundamentalTheoremOfGC:40} directly, I will attempt to build up to that point logically in a progressive fashion that is hopefully does not require the reader to make too many unjustified leaps.
...
Next we want to figure out the structure of the ``perfect'' differential for line integrals.  We can take a hint from the \R{3} vector result that we already know, namely
...
We designate \( \lrpartial \) as the vector derivative allowed to act bidirectionally, as follows
\begin{equation*}
R \lrpartial S
=
R \Bx^\mu \PD{u^\mu}{S}
...
Noting that \( du\, \Bx_u \cdot \grad = du\, \Bx_u \cdot \partial \), we may rewrite the scalar line integral identity \cref{eqn:fundamentalTheoremOfGC:140} as
\begin{dmath}\label{eqn:fundamentalTheoremOfGC:220}
\int_A^B dx \cdot \partial f = f(B) - f(A).
\end{dmath}
However, as our example hinted at, we can generalize this line integral to multivectors nicely.  That generalization takes the following form
\maketheorem{Fundamental theorem for line integrals.}{thm:fundamentalTheoremOfGC:10}{
Given multivector functions \( F, G \), and a single parameter curve \( x(u) \) with line element \( d^1 \Bx = \Bx_u du \), the differential
\begin{equation*}
